...
async def create_submission(body: SubmissionCreate):
    ...
    # Fetch quotes from all registered carriers
    quotes: list[Quote] = []
    for carrier in CARRIERS:
        try:
            quote = await carrier.get_quote(submission)
...
async def bind_submission(submission_id: str, body: BindRequest):
    ...
    # Find the right carrier client and bind
    for carrier_client in CARRIERS:
        if isinstance(carrier_client, CarrierAClient) and body.carrier == "carrier_a":
            return await carrier_client.bind_quote(body.quote_id)
        if isinstance(carrier_client, CarrierBClient) and body.carrier == "carrier_b":
            return await carrier_client.bind_quote(body.quote_id)
